...
class CategoryEmbedding(nn.Module):
    def __init__(self, d_model, categories, do_sum=True):
        super(CategoryEmbedding, self).__init__()

        self.embeddings = nn.ModuleDict({
            cat: nn.Embedding(categories[cat], d_model) for cat in categories
        })
        self.do_sum = do_sum

    def forward(self, x):
        # x: int tensor 
        x = [self.embeddings[key](x[..., i]) for i, key in enumerate(self.embeddings)]
        if self.do_sum:
            return sum(x)
        return x
...
class FeaturesEmbedding(nn.Module):
    def __init__(self, d_model, categories, dropout=0.1):
        super(FeaturesEmbedding, self).__init__()

        self.position_embedding = PositionalEmbedding(d_model=d_model)
        self.category_embedding = CategoryEmbedding(d_model=d_model, categories=categories)
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, x, x_mark):
        x = x + self.category_embedding(x_mark) + self.position_embedding(x)
        return self.dropout(x)
